...
(H1) Assumptions contained by users of the Hadoop FileSystem APIs
...
(H1) Core requirements of a Hadoop Compatible Filesystem
...
(H1) Formal specification of a Hadoop filesystem
...
(H1) org.apache.hadoop.fs.FileSystem
...
(H2) FileStatus getFileStatus(Path P)
...
(H2) boolean (Path F, FsPermission Permission )
...
(H2) FSDataOutputStream create(Path f, ...)
...
(H2) FSDataOutputStream append(Path P, int bufferSize, Progressable progress)
...
(H2) FSDataInputStream open(Path f, int bufferSize)
...
*** HDFS actions
  ...
  MAY throw `UnresolvedPathException` when attempting to traverse
  symbolic links
  
  throws IOException("Cannot open filename " + src); if it 
...
(H2) `FileSystem.delete(Path P, boolean recursive)`
...
(H2) `FileSystem.rename(Path S, Path D)`
...
(H2) FileSystem.listStatus(Path P, PathFilter Filter) 
...
(H2)  getFileBlockLocations(FileStatus F, int S, int L)
...
(H2)  getFileBlockLocations(Path P, int S, int L)
...
(H2)  getDefaultBlockSize(Path P), getDefaultBlockSize();
...
(H1) FSDataOutputStream extends DataOutputStream implements Syncable
...
(H1) InputStream, Seekable and PositionedReadable
